...
==========================================
> The ElderDelp aspect of the project, try to add debugging feedback so that sites using Capta can be supported.
>
> Signin will not work on:
>
>  - HomeDepot.com
>  - Roku.com
>  - iplocation.net (this one makes sense because the restrictor acts like a firewall, so the IP address will not be the machine's address.
...
* **[Virtual Reality API](https://immersive-web.github.io/webvr/spec/1.1/)** and **[Mixed Reality](https://immersive-web.github.io/webxr/)** APIs allow web pages to access information on connected VR sets and similar devices. Fingerprinters were already [observed to use the attached devices to identify the browser](https://github.com/uiowa-irl/FP-Inspector/blob/master/Data/potential_fingerprinting_APIs.md). As we expect that the majority of the users does not have VR sets connected, we provide only a single mitigation - the wrapped APIs returns an empty list and we disable Web XR completely.
